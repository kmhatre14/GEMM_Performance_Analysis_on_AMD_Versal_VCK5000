    ...
    mm_k1_B4_L0() {
      ...
      for (int j=0; j<L0_NUM_PACK_IN; j++){
        for (int i=0; i<L0_NUM_KERNELS_PER_PAC;i++){
          ...
          connect<pktstream,window<L0_h1*L0_w1*L0_Byte>>(in0[i], mm[i+j*L0_NUM_KERNELS_PER_PAC].in[0]);
          // connect<pktstream,window<L0_w1*L0_w2*L0_Byte>>(sp_b0[j].out[i], mm[i+j*L0_NUM_KERNELS_PER_PAC].in[1]);
          connect<pktstream,window<L0_w1*L0_w2*L0_Byte>>(in1[i], mm[i+j*L0_NUM_KERNELS_PER_PAC].in[1]);
        }
      }
