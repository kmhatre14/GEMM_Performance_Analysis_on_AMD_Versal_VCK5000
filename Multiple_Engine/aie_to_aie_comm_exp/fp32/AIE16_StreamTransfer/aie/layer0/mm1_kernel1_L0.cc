...
void mm1_kernel1_L0(input_window_float* __restrict matA, 
        input_window_float* __restrict matB,
        input_stream_float* __restrict acc_in,
	...
	{	
		...
		chess_loop_range(L0_boundary_j,){
			v8float  acc0;
			acc0 = upd_v(acc0, 0, readincr_v4(acc_in));
			acc0 = upd_v(acc0, 1, readincr_v4(acc_in));
			v8float  acc1;
			acc1 = upd_v(acc1, 0, readincr_v4(acc_in));
			acc1 = upd_v(acc1, 1, readincr_v4(acc_in));
			int jump=L0_h1;
			if (j==L0_judge_j){
				if(i==L0_judge_i){
					jump=8;
			...
			acc0 = fpmac(acc0,buf_matA,8,0x76543210,ext_w(buf_matB,1),3,0x0);
			writeincr_v4(matC, ext_v(acc0, 0));
			writeincr_v4(matC, ext_v(acc0, 1));
			buf_matA = upd_w(buf_matA,0,window_read_v8(matA));
			window_incr(matA,L0_h1);
			buf_matB = upd_v(buf_matB,0,window_read_v4(matB));
			window_incr(matB,L0_w1);
			acc1 = fpmac(acc1,buf_matA,8,0x76543210,ext_w(buf_matB,1),7,0x0);
			writeincr_v4(matC, ext_v(acc1, 0));
			writeincr_v4(matC, ext_v(acc1, 1));
			buf_matB = upd_v(buf_matB,1,window_read_v4(matB));
			window_decr(matB,L0_jump_B0);	
		}
	}
