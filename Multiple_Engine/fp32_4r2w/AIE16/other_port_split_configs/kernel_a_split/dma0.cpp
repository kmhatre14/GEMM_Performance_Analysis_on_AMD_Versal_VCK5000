...
#pragma HLS dependence variable = b_buf type = intra false
                                ...
                                // printf("B pos2 : %d pos1 : %d pos0 : %d \n", pos2,pos1,pos0);
                                // printf("Bm : %d \n", m);
                                // for (int i = 0; i < 16; i++)
                                // {
                                //     printf("B : %f ", (float)temp_data(i * 32 + 31, i * 32));
                                // }
                                // printf("\n");
                            }
                        }
...
#pragma HLS dependence variable = c_buf type = intra false
                                ...
                                // printf("pos3 : %d pos2 : %d pos1 : %d pos0 : %d \n", pos3,pos2,pos1,pos0);
                                // printf("pos2 : %d ", pos2);
                                // printf("pos1 : %d ", pos1);
                                // printf("pos0 : %d \n", pos0);
                                // printf("i : %d \n", i);
                                // for (int i = 0; i < 16; i++)
                                // {
                                //     printf("Scd : %f ", (float)temp_data(i * 32 + 31, i * 32));
                                // }
                                // printf("\n");
...
void compute(axis_stream_A &dataA0_out, axis_stream_A &dataA1_out, axis_stream_B &dataB_out, axis_stream_C &dataC_in,
...
#pragma HLS interface m_axi offset = slave bundle = gmem2 port = inb max_read_burst_length = 16 num_read_outstanding = 64
#pragma HLS interface s_axilite bundle = control port = inb
#pragma HLS interface m_axi offset = slave bundle = gmem3 port = out0 max_write_burst_length = 16 num_write_outstanding = 64
...
#pragma HLS dataflow
    ...
    compute(dataA0_out, dataA1_out, dataB_out, dataC_in,
